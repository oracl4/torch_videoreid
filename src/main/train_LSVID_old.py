...
# Local features path
localfeat2_path = '../../features/input/LSVID/previous/trainval/trainval_split_2/'
localfeat4_path = '../../features/input/LSVID/previous/trainval/trainval_split_4/'
...
for epoch in range(0, n_epochs+1):
    ...
    if((epoch) % 10 == 0):
        ...
        TrainingModel.eval()
        with torch.no_grad():
            evaluator = test_LSVID_old.Evaluator(evaluation_path, TrainingModel, dataset_name, experiment_name, epoch, batch_size, n_features, n_class, n_frames, n_partitions, cuda_device)
            result_map, result_r1 = evaluator.get_evaluation()

        # Tensorboard
        writer.add_scalar('test_map', result_map, epoch)
        writer.add_scalar('test_r1', result_r1, epoch)
