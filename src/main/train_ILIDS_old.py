...
for epoch in range(0, n_epochs+1):
    ...
    if((epoch) % 10 == 0):
        ...
        TrainingModel.eval()
        with torch.no_grad():
            evaluator = test_ILIDS_old.Evaluator(evaluation_path, TrainingModel, dataset_name, experiment_name, epoch, batch_size, n_features, n_class, n_frames, n_partitions, cuda_device)
            R1, R5, R20 = evaluator.get_evaluation()

        # Tensorboard
        writer.add_scalar('test_r1', R1, epoch)
        writer.add_scalar('test_r5', R5, epoch)
        writer.add_scalar('test_r20', R20, epoch)
